...
def plot_sdwi():
    ...
    for idx, month in enumerate(MONTHS):
        ...
        sdwi_rgb = np.stack([sdwi_norm] * 3, axis=-1)

        # Add SDWI image to the plotly subplot
        fig.add_trace(go.Image(z=sdwi_rgb), row=row, col=col)

    # Update layout
    fig.update_layout(
        title_text="Monthly Sentinel-1 Radar Images (SDWI Index)",
        height=900,
        width=1200,
    )
    ...
    fig.write_html("results/monthly_sdwi.html")
